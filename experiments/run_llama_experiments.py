"""
Poison detection experiments for LLaMA models.

This script runs influence-based poison detection on LLaMA models to compare
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Poison detection experiments for LLaMA models'
    )
    parser.add_argument('--task', type=str, default='polarity',
                      help='Task name (polarity, sentiment, etc.)')
    parser.add_argument('--model', type=str,
                      default='meta-llama/Llama-3.2-1B',
                      help='LLaMA model variant (meta-llama/Llama-3.2-1B, meta-llama/Llama-3.2-3B, etc.)')
    ...
    parser.add_argument('--output_dir', type=str,
                      default='experiments/results/llama',
...
def create_causal_lm_dataset(samples, tokenizer, max_length=256):
    """
    Create dataset for causal language models (LLaMA).
...
def load_llama_model(model_name: str, use_8bit: bool = False, use_4bit: bool = False):
...
def run_experiments(args):
    """Run complete poison detection experiments for LLaMA."""
    print("="*80)
    print("LLAMA POISON DETECTION EXPERIMENTS")
    print("="*80)
    ...
    influence_scores, factor_time, score_time = compute_influence_scores(
        ...
        train_loader=train_loader,
        test_loader=test_loader,
        analysis_name=f"llama_{args.task}",
        output_dir=output_dir,
        damping_factor=args.damping_factor
    )
