...
class TKTrain:
    def __init__(self, 
                 train_fn: Callable[[PyTree, PyTree, jax.random.PRNGKey, jnp.ndarray, jnp.ndarray], StepOutput], 
                 params: PyTree, 
                 opt_state: PyTree, 
                 tokenizer: Any, 
                 param_spec: PyTree
...
class TKInference:
    def __init__(self, 
                 generate_fn: Callable[[PyTree, jax.random.PRNGKey, jnp.ndarray, Dict[str, Any]], jnp.ndarray], 
    ...
    def generate_from_tokens(self, in_tokens: jnp.ndarray, rng_key: jax.random.PRNGKey, 
    ...
    def generate_from_str(self, in_strs: List[str], max_input_length: int, 
                          rng_key: jax.random.PRNGKey, **generation_kwargs: Dict[str, Any]) -> List[str]:
...
class TKTrainConfig(ConfigScript):
    ...
    def unroll(self, metaconfig: MetaConfig) -> Tuple[TKTrain, TKInference, FlaxPreTrainedModel, Optional[Mesh]]:
        ...
        # Specifies how to split model parameters between devices
        ...
        # Get PartitionSpec for opt_state
        def get_opt_spec(x):
            if isinstance(x, (dict, FrozenDict,)):
                return param_spec
            return None
        
        if isinstance(self.optim, (AdamWConfig, AdaFactorConfig)):
            ...
            if isinstance(self.optim, AdaFactorConfig):
                opt_state_spec = opt_state_spec._replace(inner_opt_state=None)
        else:
            raise NotImplementedError

        # Pjit the get_initial_state function to shard params and init optimizer state in sharded way
        if self.pjit:
        ...
        # Define evaluation loss function
        def log_prob_fn(params, input_ids, decoder_input_ids):
            ...
            log_probs = -(softmax_cross_entropy_with_integer_labels(
            ...
            return train_interface, inference_interface, model, mesh
