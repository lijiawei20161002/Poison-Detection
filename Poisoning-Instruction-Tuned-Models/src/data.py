...
from core_cloud import block_tokens, prepend_pad, prepend_ul2_autoregressive_sentenal
...
def batch_idxs(rng: Optional[jax.random.PRNGKey], data_size: int, bsize: int) -> np.ndarray:
...
def list_data_to_batch_iterator(rng: Optional[jax.random.PRNGKey], dataset: Any, bsize: int, postproc_f: Optional[Callable]=None, trunc: bool=True) -> Iterator:
...
def dataloader(rng: Optional[jax.random.PRNGKey], dataset: Union[Dataset, IterableDataset], 
