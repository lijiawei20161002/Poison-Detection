        ...
        for batch in train_loader:
            ...
            with torch.no_grad():
                if score_args.compute_per_module_scores:
                    ...
                    for module in cached_module_lst:
                        factor = module.get_factor(factor_name=PAIRWISE_SCORE_MATRIX_NAME)
                        if pairwise_scores is None:
                            pairwise_scores = torch.zeros_like(
                                factor, requires_grad=False
                            )
                        if factor.shape != pairwise_scores.shape:
                            padding_size = pairwise_scores.shape[1] - factor.shape[1]
                            if padding_size > 0:
                                factor = F.pad(factor, (0, padding_size), mode='constant', value=0)
                            else:
                                pairwise_scores = F.pad(pairwise_scores, (0, -padding_size), mode='constant', value=0)
                        try:
                            pairwise_scores.add_(factor)
